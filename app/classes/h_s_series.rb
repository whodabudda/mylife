...
class HSSeries
  attr_accessor :theMetric, :current_user, :dataset, :stddev, :mean
  attr_reader :event
    ...
    def stddevCount(v)
     if  (@theMetric.series_type == 'event')
        #
        #There will be some charting with very little variation. For example, if a person's habit is 
        #to jog the same amount of time every outing, then the stddev will always be near 0, and 
        #the value - mean will be very small. But we can't have a divide by zero.
        #to try to address this, will return zero if either the stddev is zero
